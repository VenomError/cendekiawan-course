        ...
        <div class="container">
            <div class="row justify-content-center">
                @foreach ($riwayats as $pendaftar)
                    @php
                        $pendaftar->load(['jadwals']);
                    @endphp

                    @foreach ($pendaftar->jadwals as $jadwal)
                                    ...
                                    <div class="blog-two__item blog-two__item--list">
                                        <div class="blog-two__content">
                                            <div class="blog-two__top-meta">
                                                <div
                                                    class="blog-two__cats bg-{{ pendaftarStatusColor($pendaftar->status) }} rounded">
                                                    <a class="text-white">{{ $pendaftar->status }}</a>
                                                </div>
                                                <div class="blog-two__date">
                                                    <i class="icon-clock"></i>
                                                    {{ $jadwal->start_datetime?->format('D d M, Y h:i a') }}
                                                </div>
                                            </div>
                                            <h3 class="blog-two__title">
                                                <a
                                                    href="{{ route('landing.kursus.detail', ['slug' => $jadwal?->kursus?->slug]) }}">
                                                    {{ $jadwal?->kursus?->name }}
                                                </a>
                                            </h3><!-- /.blog-title -->
